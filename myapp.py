...
st.set_page_config(
    page_title="DataWorkbench (A Data Analytics Web Portal)",
    page_icon= '📊'
)

st.title(":rainbow[DataWorkbench]")
st.subheader(':gray[A Smart Data Analytics Application]')
...
if (file!=None):
    ...
    with tab3:
        st.subheader(':gray[Datatypes of Columns of the dataset]')
        st.dataframe(data.dtypes)

    with tab4:
        st.subheader(':gray[Column Names in the dataset]')
    ...
    st.write('The Groupby lets you summarize data by specific categories anf groups')
    with st.expander('Group by your columns'):
        ...
        with col2:
            operation_col = st.selectbox('Choose your operation column',options=list(data.columns))

        with col3:
            operation = st.selectbox('Choose operation to perform',options=['sum', 'count','mean','min','max','median'])
        if(groupby_cols):
            ...
            graphs = st.selectbox('Choose you graphs',options=['line','bar','scatter','pie','sunburst'])
            if(graphs=='line'):
